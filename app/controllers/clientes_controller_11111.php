...
class ClientesController extends AppController {
	...
	function index() {
               ...
               ( !empty($this->data['Cliente']['cpf']) ) ? $cpf = 'Cliente.cpf LIKE \'%'.$this->data['Cliente']['cpf'].'%\'' : $cpf = '';
                $this->paginate = array('fields' => array('id', 'nome', 'cpf', 'email', 'created'), 'conditions' => array($nome, $cpf));
		...
		if (!empty($this->data)) {
			$this->Cliente->create();                        
				...
				$this->addMessageError(__('Ocorreu um erro ao salvar o Cliente. Tente novamente.', true));
                    ...
                    $planos = $this->Cliente->Plano->find('list' , array('order'=>'nome ASC'));
       ...
       function view($id = null) {
		//$this->Atualizaproduto->recursive = 0;
		if (!$id) {
			$this->addMessageError(__('Cliente Inválido.', true));
        ...
        function delete( $id = null) {
			...
			$this->redirect(array('action'=>'index'));
                        ...
                        //$this->Cliente->cancelaClienteId($data);
